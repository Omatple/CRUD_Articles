...
if (!$id || !$article = Article::getArticleById($id)) UtilsSession::redirectTo("articles.php");
...
if (isset($_POST["name"])) {
    ...
    if (!$hasErrors && ArticleValidator::validateIfNameTaken($name, (int) $article["id"])) $hasErrors = true;
    ...
    if ($hasErrors) UtilsSession::redirectTo($_SERVER["PHP_SELF"] . "?id=" . $article["id"]);
    (new Article)
        ->setName($name)
