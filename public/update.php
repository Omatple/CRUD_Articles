...
if (!$id || !$article = Article::getArticleById($id)) Redirector::redirectTo("articles.php");
...
if (isset($_POST["name"])) {
    ...
    $hasErrors = false;
    if (!ArticleValidator::isNameLengthValid($name)) $hasErrors = true;
    if (!$hasErrors && ArticleValidator::isNameTaken($name, (int) $article["id"])) $hasErrors = true;
    if (!ArticleValidator::isDescriptionLengthValid($description)) $hasErrors = true;
    if (!ArticleValidator::isAvailabilityValid($availability)) $hasErrors = true;
    if (!ArticleValidator::isCategoryValid($categoryId)) $hasErrors = true;
    if ($hasErrors) Redirector::redirectTo($_SERVER["PHP_SELF"] . "?id=" . $article["id"]);
    (new Article)
        ->setName($name)
...
$checkedNo = ($article["availability"] === "NO") ? " checked" : "";
$checkedYes = ($article["availability"] === "YES") ? " checked" : "";
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="bg-white dark:bg-gray-900">
    <section class="bg-white dark:bg-gray-900">
        <div class="mt-8 py-8 px-4 mx-auto max-w-2xl lg:py-16">
            ...
            <form action="<?= $_SERVER["PHP_SELF"] . "?id=" . $article["id"] ?>" method="POST" novalidate>
                <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                    <div class="sm:col-span-2">
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Article Name</label>
                        <input type="text" name="name" id="name" value="<?= $article["name"] ?>" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Type product name" required="">
                        <?= ErrorDisplay::displaySessionError("name") ?>
                    </div>
                    <div class="w-full">
                        ...
                        <ul class="items-center w-full text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg sm:flex dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                            <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                <div class="flex items-center ps-3">
                                    <input id="availability" name="availability" type="radio" value="yes" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" <?= $checkedYes ?>>
                                    <label for="availability" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Available</label>
                                </div>
                            </li>
                            <li class="w-full border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                                <div class="flex items-center ps-3">
                                    <input id="availability" name="availability" type="radio" value="no" name="list-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" <?= $checkedNo ?>>
                                    <label for="availability" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Not Available</label>
                                </div>
                            </li>
                        </ul>
                        <?= ErrorDisplay::displaySessionError("availability") ?>
                    </div>
                    <div>
                        ...
                        <select name="categoryId" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                            ...
                            foreach ($categories as $category) {
                                $selectedCategory = ($category['id'] === $article["category_id"]) ? " selected=''" : "";
                                echo "<option value='{$category['id']}' $selectedCategory>{$category['name']}</option>";
                            }
                            ?>
                        </select>
                        <?= ErrorDisplay::displaySessionError("category") ?>
                    </div>
                    <div class="sm:col-span-2">
                        <label for="description" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Description</label>
                        <textarea name="description" rows="8" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Your description here"><?= $article["description"] ?></textarea>
                        <?= ErrorDisplay::displaySessionError("description") ?>
                    </div>
                </div>
                <button type="submit" class="inline-flex items-center px-5 py-2.5 mt-4 sm:mt-6 text-sm font-medium text-center text-white bg-green-800 rounded-lg focus:ring-4 focus:ring-green-300 dark:focus:ring-green-950 hover:bg-green-900">
                    Update article
                </button>
